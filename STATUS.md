...
| C12 | Landing page ported from `web/design/landing` into `web/site` | C | C1 | DONE | `npm run build && npm run lint` clean; verified in browser at 1440 and 375 — no console errors, no failed requests, no horizontal overflow, all 20 reveal elements visible after a jump to the bottom and after an `#faq` anchor load |
...
- (C13) Landing copy rewritten to what the code does. The Stitch design advertised FAA/Eurocontrol radar hooks, GDS ticket reissue, Apple Wallet push, WhatsApp/SMS dispatch, Concur/Navan integrations, and invented figures (14,000 flyers, 99.8% resolution, 450+ airlines, sub-85ms). None of it exists. The page now describes the real five stages and names the source files; the trust strip lists the four providers actually wired; the FAQ states what is mocked and ends on "does it send WhatsApp or SMS? No." `Log in` is gone from the nav because there is no auth, and the footer's twelve dead links are replaced by in-page anchors and the dashboard.
- (C13) `ConfirmationCard`, `ApprovalModal` and `HotelPolicyCard` now treat `hotel_change`, `chosen_option_id` and `total_cost_delta_inr` as nullable, matching `approvals._response()`. `plan.hotel_change.required` threw on Ananya — no hotel row, fare under her auto-approve threshold, so the orchestrator takes the AUTO path and renders the confirmation card on the seed's designated happy path. `formatDelta` handles null centrally and returns "Not calculated" rather than `−₹0`, which claimed "no cost change" for a figure never computed.
- (C12) `ui-ux/` is byte-identical to `web/design/` (stitch screens + landing). It is the same parked reference under two names; the port reads from `web/design/landing/`.
- (C12) The landing page stays React rather than dropping `site.html` in as a static file: the CTA targets come from `VITE_APP_URL`, and the scaffold, tokens and local @fontsource faces are already wired. The Tailwind CDN and its inline config are gone — the design's `font-display-lg` / `font-label-sm` aliases map onto the preset's existing `font-display` / `font-mono`, and the Jakarta aliases drop out because it is already the inherited body face.
- (C12) Material Symbols loads from Google Fonts. It is the one remote asset on the page; there is no @fontsource build of the set, and every icon sits beside its own text label, so a blocked CDN costs glyphs and not meaning.
- (C12) The window intro no longer fades its sticky layer out at the end. Fading it reveals the intro section's own background, not the hero — sticky positioning keeps the layer inside its parent — which bought ~900px of flat navy between the intro and the page. The open window now scrolls away instead, and `#window-intro`'s background matches the bottom of the sky gradient so the hand-off is seamless.
- (C12) Scroll reveals use the same scroll-listener `useReveal` as the app, not IntersectionObserver. The nav has three in-page anchors, and an observer never fires for an element a jump skips past, which would leave sections permanently at `opacity: 0`.
- (C12) The header carries `--nav-visibility` alongside `--nav-opacity`. At `opacity: 0` its seven links stayed clickable and in the tab order over the intro; `visibility: hidden` is what actually removes them.
