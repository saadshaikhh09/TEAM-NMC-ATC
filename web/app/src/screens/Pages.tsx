...
export function OverviewPage() {
  ...
  const trip = state.data?.[0]
  const tripId = trip?.id
  useEffect(() => { if (tripId) void apiTravelService.getTimeline(tripId).then(setActions, () => setActions([])) }, [tripId])
  const simulate = async (kind: SimulationKind) => {
    ...
    state.reload()
    if (trip) setActions(await apiTravelService.getTimeline(trip.id))
  }
  return (
    <>
      ...
      {trip && <div className="page-stack"><OperationsBar live onSelectTrip={() => {}} onSimulate={simulate} trip={trip} trips={state.data ?? []} /><RadarPanel flights={trip.flights} status={trip.status} /><TripCard trip={trip} /><div className="page-actions"><Link to={`/app/trips/${trip.id}`}>Open trip details</Link><Link to={`/app/trips/${trip.id}/recovery`}>View recovery</Link></div><AgentTimeline actions={actions} /></div>}
    </>
  )
}
...
export function TripsPage() {
  ...
  return <><PageHead eyebrow="Itineraries" title="Trips under watch." copy="Trips persist in PostgreSQL and re-enter monitoring after a restart." action={<Link className="primary-action" to="/app/trips/new">Add a trip</Link>} />
    ...
    <div className="trip-list">{state.data?.map((trip) => <div key={trip.id}><TripCard trip={trip} /><div className="page-actions"><Link to={`/app/trips/${trip.id}`}>Details</Link><Link to={`/app/trips/${trip.id}/recovery`}>Recovery</Link></div></div>)}</div>
  </>
}
  ...
  const submit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const payload = {
        ...
        outbound: { carrier: value('carrier'), flight_number: value('flight_number'), origin: value('origin'), destination: value('destination'), scheduled_departure: value('departure'), scheduled_arrival: value('arrival'), origin_timezone: value('origin_timezone') || null, destination_timezone: value('destination_timezone') || null, booking_reference: value('booking_reference') || null },
        ...
        constraints: { max_fare_inr: value('max_fare_inr') ? Number(value('max_fare_inr')) : null, max_stops: Number(value('max_stops') || 1), auto_approve_under_inr: value('auto_approve') ? Number(value('auto_approve')) : null, cabin: 'economy' },
      }
...
export function ProfilePage({ account, onLogout }: { account: Account; onLogout: () => Promise<void> }) {
  ...
  return <><PageHead eyebrow="Account" title="Profile & safeguards." /><section className="profile-page"><div className="profile-identity"><img alt="ATC logo" src="/assets/atc-logo.png" /><div><h2>{account.name}</h2><p>{account.email}</p></div></div><dl><div><dt>Fare cap</dt><dd>{constraints?.max_fare_inr ? `₹${constraints.max_fare_inr.toLocaleString('en-IN')}` : 'Not set'}</dd></div><div><dt>Maximum stops</dt><dd>{constraints?.max_stops ?? 'Not set'}</dd></div><div><dt>Cabin</dt><dd>{constraints?.cabin ?? 'Not set'}</dd></div><div><dt>Auto-approve</dt><dd>{constraints?.auto_approve_under_inr ? `₹${constraints.auto_approve_under_inr.toLocaleString('en-IN')}` : 'Not set'}</dd></div></dl><p>Safeguards are applied by deterministic policy code. The optional model only narrates the decision.</p><button onClick={() => void onLogout()} type="button">Log out</button></section></>
}
